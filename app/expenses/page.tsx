...
export default function ExpensesPage() {
  ...
  return (
    <div className="space-y-4 w-full flex flex-col h-auto lg:h-[calc(100vh-90px)]">
      {/* 1. 상단 타이틀 바 */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 bg-white p-4 sm:p-5 rounded-2xl border border-slate-100 shadow-xs shrink-0">
        <div>
        ...
        {selectedTrip && (
          <div className="flex items-center gap-2 pt-1 sm:pt-0 shrink-0 w-full sm:w-auto">
            <button
              onClick={() => setIsBudgetModalOpen(true)}
              className="flex-1 sm:flex-initial justify-center bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs sm:text-sm font-bold px-3 py-2.5 rounded-xl transition flex items-center gap-1.5 cursor-pointer whitespace-nowrap"
            >
              <Wallet size={15} /> 총 환전금 입력
            </button>
            <button
              onClick={() => setIsExpenseModalOpen(true)}
              className="flex-1 sm:flex-initial justify-center bg-blue-600 hover:bg-blue-700 text-white text-xs sm:text-sm font-bold px-3.5 py-2.5 rounded-xl transition flex items-center gap-1.5 shadow-sm cursor-pointer whitespace-nowrap"
            >
              <Plus size={15} /> 지출 추가
            </button>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 flex-1 min-h-0">
        ...
        <div className="lg:col-span-4 bg-white rounded-2xl border border-slate-100 shadow-xs p-4 flex flex-col min-h-[180px] max-h-[250px] lg:max-h-none lg:min-h-0">
          <div className="flex items-center justify-between border-b border-slate-100 pb-3 mb-3 shrink-0">
            <h2 className="text-sm font-bold text-slate-800">여행 선택</h2>
          ...
          <div className="space-y-2.5 overflow-y-auto flex-1 pr-1">
            {loading ? (
              <p className="text-xs text-slate-400 py-8 text-center">불러오는 중...</p>
            ) : trips.length === 0 ? (
              <p className="text-xs text-slate-400 py-8 text-center">등록된 여행이 없습니다.</p>
            ) : (
              trips.map((trip) => {
        ...
        {/* 우측 경비 현황 */}
        <div className="lg:col-span-8 bg-white rounded-2xl border border-slate-100 shadow-xs p-4 sm:p-5 flex flex-col min-h-[450px] lg:min-h-0 space-y-4">
          {selectedTrip ? (
            <div className="flex flex-col h-full space-y-4">
              {/* 🌟 카드 스타일링 보정: 모던한 슬레이트 톤 & 선명한 타이포그래피 */}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {/* 1. 현금 잔액 게이지 카드 */}
                <div className="p-4 bg-slate-50 border border-slate-200/80 rounded-2xl space-y-2.5">
                  <div className="flex items-center justify-between text-xs font-bold text-slate-600">
                    <span className="flex items-center gap-1.5"><Coins size={16} className="text-amber-500" /> 남은 환전 잔액 (현금+트래블)</span>
                ...
                <div className="p-4 bg-blue-50/40 border border-blue-100 rounded-2xl space-y-2.5">
                  <div className="flex items-center justify-between text-xs font-bold text-blue-900">
                    <span className="flex items-center gap-1.5"><CreditCard size={16} className="text-blue-600" /> 총 지출 내역</span>
              ...
              {/* 🌟 카테고리별 사용금액 카드 (4열 배치 & 정돈된 서체 및 색상) */}
              <div>
                <h3 className="text-xs font-bold text-slate-400 mb-2">카테고리별 지출 요약</h3>
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-2.5">
                  {categories.map((cat) => {
                    ...
                    return (
                      <div key={cat.name} className={`p-3 sm:p-3.5 rounded-xl border border-slate-100 flex items-center gap-2.5 ${cat.bg}`}>
                        <CategoryIcon size={18} className={`shrink-0 ${cat.iconColor}`} />
                        <div className="min-w-0 flex-1">
                          <p className="text-[11px] font-bold text-slate-500">{cat.name}</p>
                  ...
                  {expenses.length === 0 ? (
                    ...
                    expenses.map((item) => {
                      ...
                      return (
                        <div
                          key={item.id}
                          className="p-3 bg-slate-50/70 hover:bg-white border border-slate-100 hover:border-slate-200 rounded-xl shadow-2xs flex items-center justify-between gap-3 transition group"
                        >
                          <div className="space-y-1 min-w-0 flex-1">
                            <div className="flex items-center gap-1.5">
                              ...
                              <span className="text-[11px] text-slate-400 font-medium">{item.expense_date}</span>
                            </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button
                    ...
                    className={`py-2.5 px-2 rounded-xl text-xs font-bold border transition cursor-pointer flex flex-col items-center justify-center gap-0.5 min-h-[52px] ${
                      paymentMethod === '현금'
